...
int BPF_PROG(aegis_exec_guard, struct linux_binprm *bprm, int ret)
{
    ...
    /*
     * Do NOT consume the token here! bprm_check_security can be called
     * multiple times for a single execve (e.g. scripts, binfmt_misc, or
     * recursive resolutions). Deleting it on the first call causes the
     * second call to fail with EPERM. Rely on the 5-second expiration instead.
     */
    return 0;
}
