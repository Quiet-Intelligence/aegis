...
if command -v apt-get >/dev/null 2>&1; then
    sudo apt-get update
    sudo apt-get install -y clang llvm libbpf-dev make golang wget python3 python3-pip python3-numpy python3-sklearn python3-matplotlib
    ...
    sudo pacman -Sy --needed --noconfirm clang libbpf make go wget bpf python python-pip python-numpy python-scikit-learn python-matplotlib
elif command -v dnf >/dev/null 2>&1; then
    sudo dnf install -y --skip-unavailable clang llvm libbpf-devel make golang wget bpftool python3 python3-pip python3-numpy python3-scikit-learn python3-matplotlib "kernel-devel-$(uname -r)"
else
